...
app.post("/signup", (req, res, next) => {
    ...
    res.json({ data: [], status: "User registered successfully" })
})
...
function validateUser({ userName, password }) {
    return users.find(user => user.userName.toLowerCase() === userName && user.password.toLowerCase() === password)
}


app.get("/products", (req, res, next) => {
    console.log("products", req.query, req.query.category)
    console.log(productsArray)
    if (!req.query.category) return res.json({ data: productsArray.reverse() })
    else {
        const result = productsArray.filter(p => p.type === req.query.category.toLowerCase())
        return res.json({ data: result.reverse() })
    }
})
app.get("/products/:title", (req, res, next) => {
    console.log(req.params.title)
    const result = productsArray.filter(p => p.title.toLowerCase() === req.query.title.toLowerCase())
